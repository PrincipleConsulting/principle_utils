...
extension SizeContext on BuildContext {
  ...
  double scaleBaseMin({double? baseHeight, double? baseWidth, Curve? curve}) {
    ...
    if (curve == null) {
      return min(scaleBaseWidth(baseWidth: baseWidth),
          scaleBaseHeight(baseHeight: baseHeight));
    }
    final double scale = min(scaleBaseWidth(baseWidth: baseWidth),
        scaleBaseHeight(baseHeight: baseHeight));
    return curve.transform(scale % 1) + scale.floorToDouble();
  }
  ...
  double scaleBaseMax({double? baseHeight, double? baseWidth, Curve? curve}) {
    ...
    baseWidth ??= this.baseWidth;
    if (curve == null) {
      return max(scaleBaseWidth(baseWidth: baseWidth),
          scaleBaseHeight(baseHeight: baseHeight));
    }
    final double scale = max(scaleBaseWidth(baseWidth: baseWidth),
        scaleBaseHeight(baseHeight: baseHeight));
    return curve.transform(scale % 1) + scale.floorToDouble();
  }
}
